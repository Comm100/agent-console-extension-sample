...
var sdkType = 'extension'; // extension & iframe
var iframeId = 'agent-console-iframe';
...
var _ = {
  ...
  notify: function notify(eventName) {
    ...
    if (eventList) {
      ...
      for (; i < len; i += 1) {
        status = eventList[i].apply(this, rest);
      ...
      rest.forEach(function (item) {
        if (item.action === 'on' && item.eventTrace) {
          var messageObj = {
            api: item.eventTrace,
            action: 'on-status',
            params: status,
            msgId: item.eventTrace
          };
          postMessage(JSON.stringify(messageObj));
        }
      });
    } else {
      toBeNotify.push({
    ...
    if (eventName.indexOf(EVENT_PREFIX + '_') === 0) {
...
var AgentConsoleAPI = {
  onReady: function onReady(callback) {
    window.addEventListener('message', function (e) {
      if (sdkType === 'extension' && e.source !== window.parent) {
        return;
      }
      if (e.data) {
    ...
    var handler = function handler(data) {
      AgentConsoleAPI.config = data;
      if (callback) callback();
    };
    handler({ isWeb: true });
  },
  init: function init(data) {
    if (data && data.sdkType) {
      if (sdkType !== 'extension' && sdkType !== 'iframe') {
        return;
      }
      sdkType = data.sdkType;
      if (sdkType === 'iframe' && data.iframeId) {
...
window["Comm100AgentConsoleAPI"] = AgentConsoleAPI;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__finally__ = __webpack_require__(3);
